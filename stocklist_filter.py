...
class stockFilter:
    ...
    def __init__(self,manager,master_list):
        ...
        self.excluded_tickers = self.frame[~self.frame.Ticker.isin(self.filtered_tickers)].Ticker
        self.filtered_tickers.to_csv('filtered_tickers.csv',index=False)
        self.excluded_tickers.to_csv('excluded_tickers.csv',index=False)

        print("filtered tickers by avg volume (0-99th percentile)")

    def parallel_getData_wrapper(self,x,master_list):
        d = self.scanner.getData(x).Volume
        if d.empty:
            return
        else:
            data_mean = np.mean(d)
            new_row = {'Ticker':x, 'AvgVol':data_mean}
